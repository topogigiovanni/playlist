...
app.factory('Search', function($rootScope, $http) {
    var factory = {};
    var resolveData = function(r, isSuccess){
      console.log('resolveData', r, isSuccess);
      factory.resultItems = r.items;
      $rootScope.$broadcast('Search.DateReceived', factory);
      //$rootScope.$apply();
    };
    var doSearch = function(term){
      ...
      if(term.length > 2){
        $.getJSON('https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q='+term+'&key='+YTKey, resolveData);
        // $http.get('https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q='+term+'&key='+YTKey).
        //       success(function(data, status, headers, config) {
        //         //$scope.posts = data;
        //         factory.resultItems = data.items;
    ...
    factory.doSearch =  _.throttle(doSearch, 290);
    factory.resultItems = [];
    factory.modal = {};
