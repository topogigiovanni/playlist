      ...
      if(items.data.length){
        items.data.forEach(function(el, index){
          ...
          if(!thumb.length){
            ...
            thumb = thumb[0].link;
          list.push(new SearchResultItem(id, 'vimeo', title, thumb));
        });
      }
    ...
    var _callback = function(items, success){
      ...
      if(items.items.length){
        items.items.forEach(function(el, index){
          ...
          list.push(new SearchResultItem(id, 'youtube', title, thumb));
        });
      }
      callback(list, success);
    };
...
app.factory('Search', function($rootScope, $http, YoutubeSearch, VimeoSearch) {
    ...
    var doSearch = function(term){
      ...
      if(term.length > 2){
        ...
        //$.getJSON('https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q='+term+'&key='+KEYS['youtube'], bulkResultItems);
