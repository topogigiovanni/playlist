...
app.controller('VideoPlayerController', ['$scope', '$rootScope', 'Player', 'CurrentVideo', function ($scope, $rootScope, Player, CurrentVideo) {
 ...
 // angular.extend($scope, Player);
  ...
  $rootScope.$on('Player.Pause', function(ev, data){
    ...
    console.log('escotou Player.Pause poooos', $scope, $scope.Player);
    $scope.$apply();
  ...
  CurrentVideo.instance.addEventListener('onStateChange', function(e,b,c){
      console.log('intoo player listenrr 333', e,b,c);
      if(e.data === 1){
        //$scope.Player.isPlaying  = true;
        $scope.isPlaying  = true;
        angular.extend($scope, {isPlaying: true});
        $scope.Player.changeToPlay();
